...
const LoginUser = (req: Request, res: Response) => {
    ...
    Users.findOne({ email: body.email }, (error: CallbackError, userDB: { password: string; attempts: number }) => {
        ...
        if (userDB.attempts >= 3) {
            Users.findOneAndUpdate({ email: body.email }, {
                $set: {
        ...
        const token = jwt.sign({
            user: userDB
        }, config.TOKEN.seed, { expiresIn: config.TOKEN.token_expired });
